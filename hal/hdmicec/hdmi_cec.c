...
typedef struct hdmicec_context
{
    ...
    pthread_t thread;
    int exit_fd;
    pthread_mutex_t cec_enabled_lock;
    bool cec_enabled;
    pthread_mutex_t cec_control_enabled_lock;
...
static int hdmicec_send_message(const struct hdmi_cec_device *dev, const cec_message_t *msg)
{
    ...
    pthread_mutex_lock(&ctx->cec_enabled_lock);
    bool cec_enabled = ctx->cec_enabled;
    pthread_mutex_unlock(&ctx->cec_enabled_lock);
    if (!cec_enabled) {
...
static void hdmicec_set_option(const struct hdmi_cec_device *dev, int flag, int value)
{
    ...
    switch (flag) {
        ...
        case HDMI_OPTION_SYSTEM_CEC_CONTROL:
            pthread_mutex_lock(&ctx->cec_control_enabled_lock);
            ctx->cec_control_enabled = (value == 1 ? true : false);
            pthread_mutex_unlock(&ctx->cec_control_enabled_lock);
            break;
    }
}
...
static int cec_init(struct hdmicec_context *ctx)
{
    ...
    pthread_mutex_init(&ctx->cec_enabled_lock, NULL);
    pthread_mutex_init(&ctx->cec_control_enabled_lock, NULL);
