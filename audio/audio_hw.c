...
#define CAPTURE_PERIOD_MULTIPLIER 10  /* 20 ms */
...
static int start_input_stream(struct alsa_stream_in *in)
{
    ...
    // in->read_threshold = CAPTURE_PERIOD_COUNT * CAPTURE_PERIOD_SIZE;
    // in->config.start_threshold = CAPTURE_PERIOD_START_THRESHOLD * CAPTURE_PERIOD_SIZE;
    // in->config.avail_min = CAPTURE_PERIOD_SIZE;
...
static size_t in_get_buffer_size(const struct audio_stream *stream)
{
    ...
    size = ((size + 15) / 16) * 16;
    size_t bufsize = size * audio_stream_in_frame_size((struct audio_stream_in *)stream);
    ALOGV("in_get_buffer_size: %zu", bufsize);
    return bufsize;
}
...
{
    ALOGV("in_get_format: %d", AUDIO_FORMAT_PCM_16_BIT);
...
{
    size_t buffer_size = audio_channel_count_from_in_mask(config->channel_mask) *
                            CAPTURE_PERIOD_SIZE *
