...
int init_aec_itfe(struct alsa_audio_device *adev) {
...
int aec_set_spk_running(struct alsa_stream_out *out, bool state) {
    ALOGV("%s enter", __func__);
...
int aec_get_spk_running(struct aec_itfe *aec, bool *state) {
    ALOGV("%s enter", __func__);
    pthread_mutex_lock(&aec->lock);
