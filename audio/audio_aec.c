...
int get_pcm_timestamp(struct pcm *pcm, uint32_t sample_rate, struct aec_ts_fifo_payload *ts) {
    int ret = 0;
    if (pcm_get_htimestamp(pcm, &ts->available, &ts->timestamp) < 0) {
        ALOGE("Error getting PCM timestamp!");
        ts->timestamp.tv_sec = 0;
        ts->timestamp.tv_nsec = 0;
        return -EINVAL;
    }
    timestamp_adjust(
        &ts->timestamp,
        pcm_get_buffer_size(pcm) - ts->available,
        sample_rate);
    return ret;
}
...
void print_queue_status_to_log(struct aec_io *io) {
    ...
    ALOGV("Queue available: Audio %zd,TS %zd (count %zd)",
        q1, q2, q2/sizeof(struct aec_ts_fifo_payload));
}
...
int init_aec_reference_config(struct aec_t *aec, struct alsa_stream_out *out) {
    ...
    aec->reference.ts_fifo = fifo_init(
            FIFO_NUM_ELEMENTS * sizeof(struct aec_ts_fifo_payload),
            false /* reader_throttles_writer */);
    if (aec->reference.ts_fifo == NULL) {
...
void destroy_aec_reference_config(struct aec_t *aec) {
    ...
    memset(&aec->reference.last_timestamp, 0, sizeof(struct aec_ts_fifo_payload));
    pthread_mutex_unlock(&aec->lock);
    ALOGV("%s exit", __func__);
}
...
void destroy_aec_mic_config(struct aec_t *aec) {
    ...
    memset(&aec->mic.last_timestamp, 0, sizeof(struct aec_ts_fifo_payload));
...
/* Write from FIFOs (audio, and timestamp if present).
 * Bytes to write is specified by ts->bytes. */
static ssize_t write_to_fifo(struct aec_io *io, void *buffer, struct aec_ts_fifo_payload *ts) {
    ALOGV("%s enter", __func__);

    /* Write audio samples to FIFO */
    ssize_t written_bytes = fifo_write(io->audio_fifo, buffer, ts->bytes);

    /* Write timestamp to FIFO */
    if (io->ts_fifo != NULL) {
        fifo_write(io->ts_fifo, ts, sizeof(struct aec_ts_fifo_payload));
    }
...
int write_to_reference_fifo(struct aec_t *aec, void *buffer, struct aec_ts_fifo_payload *ts) {
    int ret = 0;
    ssize_t written_bytes = write_to_fifo(&aec->reference, buffer, ts);
    if (written_bytes < ts->bytes) {
        ALOGE("Could only write %zd of %zu bytes", written_bytes, ts->bytes);
        ret = -ENOMEM;
    }
    return ret;
}

static uint64_t get_timestamp(struct aec_io *io, ssize_t read_bytes) {
    ...
    if (io->fifo_read_write_diff_bytes < 0) {
        ...
        ssize_t ts_bytes = fifo_read(io->ts_fifo, &io->last_timestamp,
                                        sizeof(struct aec_ts_fifo_payload));

        ALOGV("Read TS bytes: %zd, expected %zu", ts_bytes, sizeof(struct aec_ts_fifo_payload));
    ...
    while (io->fifo_read_write_diff_bytes > 0) {
        ...
        fifo_read(io->ts_fifo, &io->last_timestamp, sizeof(struct aec_ts_fifo_payload));
...
/* Read from FIFOs (audio, and timestamp if present).
 * Bytes to read is specified by ts->bytes.
 * If there are fewer bytes available than requested, the read is skipped. */
static ssize_t read_from_fifo(struct aec_io *io, void *buffer, struct aec_ts_fifo_payload *ts) {
    print_queue_status_to_log(io);
    ssize_t available_bytes = fifo_available_to_read(io->audio_fifo);
    if (available_bytes < ts->bytes) {
        ALOGV("Cannot read %zu bytes from audio FIFO, only %zd available. Skipping read.",
            ts->bytes, available_bytes);
        return available_bytes;
    }
    ssize_t read_bytes = fifo_read(io->audio_fifo, buffer, ts->bytes);
    if (io->ts_fifo != NULL) {
        ts->time_usec = get_timestamp(io, read_bytes);
    }
    return read_bytes;
}
...
int process_aec(struct aec_t *aec, void* buffer, struct aec_ts_fifo_payload *ts) {
    ...
    size_t bytes = ts->bytes;

    /* Write mic data to FIFO */
    write_to_fifo(&aec->mic, buffer, ts);
    ...
    if(fifo_available_to_read(aec->mic.audio_fifo) <
        AEC_PROCESS_FRAME_UNIT * aec->mic.frame_size_bytes) {
    ...
    struct aec_ts_fifo_payload out_ts;
    out_ts.bytes = bytes;
    ssize_t read_bytes = read_from_fifo(&aec->out, buffer, &out_ts);
    if (read_bytes < bytes) {
...
static void *run_aec(void *data) {
    ...
    *ret = 0;
    ...
    /* Read from mic FIFO */
    struct aec_ts_fifo_payload mic_ts;
    mic_ts.bytes = bytes;
    ssize_t read_bytes = read_from_fifo(&aec->mic, aec->mic.buf, &mic_ts);
    if (read_bytes < bytes) {
    ...
    struct aec_ts_fifo_payload reference_ts;
    reference_ts.bytes = reference_req_bytes;
    read_bytes = read_from_fifo(&aec->reference, aec->reference_playback_format,
                                    &reference_ts);
    if (read_bytes <= 0) {
        ALOGV("Echo reference buffer empty, skipping AEC....");
        memcpy(aec->out.buf, aec->mic.buf, bytes);
        goto exit;
    } else if (read_bytes < reference_req_bytes) {
    ...
    mic_time = mic_ts.time_usec;
    reference_time = reference_ts.time_usec;
...
exit:
    ...
    struct aec_ts_fifo_payload out_ts;
    out_ts.bytes = bytes;
    (*ret) = write_to_fifo(&aec->out, aec->out.buf, &out_ts);
