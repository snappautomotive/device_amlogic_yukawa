...
#define LOG_TAG "audio_hw_yukawa_aec"
// #define LOG_NDEBUG 0
...
void print_queue_status_to_log(struct aec_itfe *aec, bool write_side) {
    ...
    if (write_side) {
        ...
        q1, q1/aec->spk_frame_size/PLAYBACK_PERIOD_SIZE, q2, q2/sizeof(struct ts_fifo_payload));
    } else {
...
void init_aec_itfe(struct alsa_audio_device *adev, ssize_t *ret) {
...
void init_aec_spk_config(struct alsa_stream_out *out, ssize_t *ret) {
	...
	*ret = 0;
	...
	aec->spk_frame_size = audio_stream_out_frame_size(&out->stream);
exit:
...
void init_aec_mic_config(struct alsa_stream_in *in, ssize_t *ret) {
	...
	*ret = 0;
	...
	aec->mic_frame_size = audio_stream_in_frame_size(&in->stream);
...
void aec_get_spk_running(struct aec_itfe *aec, bool *state) {
	ALOGV("%s enter", __func__);
	pthread_mutex_lock(&aec->lock);
	*state = aec->spk_running;
	pthread_mutex_unlock(&aec->lock);
	ALOGV("%s exit", __func__);
}
...
void write_to_spk_fifo(struct alsa_stream_out *out, void *buffer, size_t bytes, ssize_t *ret) {
	...
	*ret = 0;
    ...
    pcm_get_htimestamp(out->pcm, &spk_ts.avail, &spk_ts.tstamp);
    spk_ts.bytes = written_bytes;
    ALOGV("Speaker timestamp: %ld s, %ld nsec", spk_ts.tstamp.tv_sec, spk_ts.tstamp.tv_nsec);
...
void update_spk_timestamp(struct aec_itfe *aec, ssize_t read_bytes, uint64_t *spk_time) {
    *spk_time = 0;
    ...
    float usec_per_byte = 1E6 / ((float)(aec->spk_frame_size * aec->spk_sampling_rate));
    if (aec->read_write_diff_bytes < 0) {
        /* Reuse old time stamp */
    ...
    *spk_time = timespec_to_usec(aec->last_spk_ts.tstamp) + spk_time_offset;

    aec->read_write_diff_bytes += read_bytes;
    struct ts_fifo_payload spk_ts = aec->last_spk_ts;
    while (aec->read_write_diff_bytes > 0) {
        /* Keep popping timestamp FIFO until we get the latest timestamp */
        if (!fifo_available_to_read(aec->ts_fifo)) {
        ...
        ALOGV("Fast-forwarded timestamp by %zd bytes, remaining bytes: %zd, new timestamp (usec) %"PRIu64,
            spk_ts.bytes, aec->read_write_diff_bytes, timespec_to_usec(spk_ts.tstamp));
        aec->read_write_diff_bytes -= spk_ts.bytes;
    }
    aec->last_spk_ts = spk_ts;
}

void process_aec(struct alsa_stream_in *in, void* buffer, size_t bytes, ssize_t *ret) {
	...
	*ret = 0;
    ...
    aec_get_spk_running(aec, &spk_running);

    /* Get mic timestamp, convert to usec */
    pcm_get_htimestamp(in->pcm, &aec->last_mic_ts.avail, &aec->last_mic_ts.tstamp);
    uint64_t mic_time = timespec_to_usec(aec->last_mic_ts.tstamp);
    /* Speaker timestamp, will be updated later */
    uint64_t spk_time = 0;

    if (spk_running) {
        ssize_t spk_frame_size = aec->spk_frame_size;
        ...
        int frame, ch;
        ssize_t req_bytes = resampler_in_frames * spk_frame_size;
