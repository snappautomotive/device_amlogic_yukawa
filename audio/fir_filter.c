...
fir_filter_t* fir_init(uint32_t channels, fir_filter_mode_t mode, uint32_t filter_length,
    ...
    if (mode == FIR_PER_CHANNEL_FILTER) {
        ...
        fir->coeffs = (int16_t*)malloc(fir->filter_length * fir->channels * sizeof(int16_t));
        if (fir->coeffs == NULL) {
            ALOGE("%s: Unable to allocate memory for FIR coeffs.", __func__);
            goto exit_1;
        }
        memcpy(fir->coeffs, coeffs, fir->filter_length * fir->channels * sizeof(int16_t));
    } else {
        /* Default: same filter coeffs for all channels */
        fir->mode = FIR_SINGLE_FILTER;
        fir->coeffs = (int16_t*)malloc(fir->filter_length * sizeof(int16_t));
        if (fir->coeffs == NULL) {
            ALOGE("%s: Unable to allocate memory for FIR coeffs", __func__);
            goto exit_1;
        }
        memcpy(fir->coeffs, coeffs, fir->filter_length * sizeof(int16_t));
    }
...
void fir_process_interleaved(fir_filter_t* fir, int16_t* input, int16_t* output, uint32_t samples) {
    ...
    for (ch = 0; ch < fir->channels; ch += 2) {
...
#endif /* #ifdef __ARM_NEON */
            for (k = 0; k < fir->filter_length; k++, offset -= fir->channels) {
#ifdef __ARM_NEON
        ...
        if (use_2nd_set_coeffs) {
            p_coeff_A += (2 * fir->filter_length);
            p_coeff_B += (2 * fir->filter_length);
        }
    }
